...
class Streams extends Component {
  ...
  fetchInitData = () => {
    ...
    this.props.fetchStreams(
      `https://api.twitch.tv/helix/streams?game_id=${this.state.gameId}&first=${
        this.state.numberToFetch
  ...
  fetchMoreData = () => {
    ...
    this.setState(prevState => ({
      numberToFetch: prevState.numberToFetch + 20
    }));
    this.props.fetchStreams(
      `https://api.twitch.tv/helix/streams?game_id=${this.state.gameId}&first=${
        this.state.numberToFetch
            ...
            <div className='streamsFirstContainer'>
              ...
              <div className='streamsSecondContainer'>
                {streams.map((stream, index) => {
                  ...
                  let streamImage =
                    stream.thumbnail_url.slice(0, -20) + '320x180.jpg';
                  ...
                  return (
                    <div key={index} className='streamsThirdContainer'>
                      ...
                      <div className='streamCard'>
                        <Link to={streamUrl} params={{ gameId, streamer }}>
                          <img
                            src={streamImage}
                            alt={`${streamer} cover image`}
                          />
                        </Link>
                        <Link to={streamUrl} params={{ gameId, streamer }}>
                          <h3>{stream.title}</h3>
                        ...
                        <p>
                          {stream.viewer_count.toLocaleString()} spectateurs sur
                          <Link
                            to={`/${gameId}/${streamer}/videos`}
